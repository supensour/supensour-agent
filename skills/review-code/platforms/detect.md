...
`scripts/fetch-pull-request.sh --branch <SRC>` finds the open PR/MR whose **source branch = SRC** and
prints `{number, url, title, base}` (`{}` if none). Per-platform queries live in `lib/platform-*.sh`
