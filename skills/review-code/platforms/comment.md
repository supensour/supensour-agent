...
```
🤖 Reviewed with skill [supensour:review-code](https://github.com/supensour/supensour-agent) · suprayan@supensour · github.com/supensour/supensour-agent
```

Never hand-write this line into a comment body — `decorate_body` appends the resolved value, and
`scripts/watermark.sh` prints it for the local report. The example above is illustrative only; the
...
Dimension values come from SKILL.md → **Finding dimensions** — that list is the single source; don't coin
new ones here (they feed the fingerprint).
